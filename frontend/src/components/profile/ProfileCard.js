    ...
    <div id="profile-card" >
      <img className="cover-photo" src="https://images.unsplash.com/photo-1547626740-02cb6aed9ef8?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80" />
        <div className="details">
            {user &&
            (user.photo ? (<Avatar size={300} user={user}/>): <><div className="fallback-avatar">{user?.username[0].toUpperCase() || ""}</div><h3>{user.username}</h3></>)
            }
            <i className="fa fa-pencil-square-o" onClick={editAvatar} aria-hidden="true"></i>
            <h3>photos</h3>
